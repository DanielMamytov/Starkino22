...
class ChecklistAdapter(
    private val onClick: (ChecklistItemUi) -> Unit = {}
) : RecyclerView.Adapter<ChecklistAdapter.VH>() {

    private val items = mutableListOf<ChecklistItemUi>()

    fun submit(list: List<ChecklistItemUi>) {
        items.clear()
        items.addAll(list)
...
//        private val img: ImageView = view.findViewById(R.id.imageView) // если добавишь id

        fun bind(item: ChecklistItemUi) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val v = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_star_notes_checklist, parent, false)
        return VH(v)
    }
